...
if [[ $# -eq 1 ]]; then
    ...
    BINARY_PATH="$GOPATH/src/github.com/!metal!blockchain/metalgo/build/plugins/srEXiWaHuhNyGwPUi444Tu47ZEDwxTWrbQiuD7FmgSAQ6X7Dy"
else
...
go build -ldflags "-X github.com/shubhamdubey02/subnet-evm/plugin/evm.GitCommit=$SUBNET_EVM_COMMIT $STATIC_LD_FLAGS" -o "$BINARY_PATH" "plugin/"*.go
